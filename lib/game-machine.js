...
module.exports = (modules, initialState = {}) => {
  ...
  const koa = require('koa-compose');
  const app = new koa();
  const route = require('koa-path-match')();

  app.use(route('/moves').get(ctx => Promise.resolve('uhuuuul')))

  compose(app.middleware)(app.createContext({ method:'GET', url:'/moves', socket: {}, headers:{} }, { setHeader: ()=>{} })).then(console.log)
  // */
  ...
  const machine = {
    ...
    moveInSeq: (urls, user) => (
      urls.reduce(
        (promise, url) => promise.then(() => machine.move(url, user)),
        Promise.resolve(machine)
      )
    ),
    get: (url, user) => route(machine)('GET')(url, user),
    userForPlayer: playerId => ({
      canPlayAs: player => (playerId === player) || (playerId === _.get(player, 'id')),
      playerId,
      toJSON: () => ({ playerId }),
      toString: () => `\`${playerId}\``
    })
