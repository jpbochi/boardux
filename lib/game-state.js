...
const utils = _.assign(
  mutableState => {
    const state = immutable(mutableState);
    ...
    return {
      ...
      updateGame: (updater, ...args) => state.update('result', updater, ...args),
      updatePlayer: (id, updater, ...args) => state.updateIn(['entities', 'players', id], updater, ...args),
